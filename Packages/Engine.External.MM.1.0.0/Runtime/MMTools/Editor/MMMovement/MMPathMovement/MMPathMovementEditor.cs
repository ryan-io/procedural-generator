...
namespace MMTools.Editor
{
	...
	[InitializeOnLoad]
	public class MMPathMovementEditor : UnityEditor.Editor 
	{		
		...
		public override void OnInspectorGUI()
		{
			...
			{
				UnityEditor.Editor.DrawPropertiesExcluding (serializedObject, new string [] { "Acceleration" });
			}
		...
		protected virtual void OnSceneGUI()
	    {
			...
			for (int i=0;i<t.PathElements.Count;i++)
			{
				...
				var fmh_64_57_638223221904969470 = Quaternion.identity; Vector3 newPoint = Handles.FreeMoveHandle(oldPoint,.5f,new Vector3(.25f,.25f,.25f),Handles.CircleHandleCap);
