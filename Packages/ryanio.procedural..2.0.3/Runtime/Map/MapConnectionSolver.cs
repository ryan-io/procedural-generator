...
namespace Engine.Procedural.Runtime {
	public class MapConnectionSolver {
		...
		public Span2D<int> Connect(Span2D<int> map, List<Room> rooms, bool forceAccessibility = false) {
		...
		public MapConnectionSolver(ProceduralConfig config) {
			_random       = new Random();
...
namespace Engine.Procedural {
	public class MapConnectionSolver {
		...
		public Span2D<int> Connect(Span2D<int> map, List<Room> rooms, CancellationToken token,
		...
		void CreatePassage(Span2D<int> map, Room a, Room b, Vector2Int tileA, Vector2Int tileB) {
			Room.ConnectRooms(a, b);
			// Debug.DrawLine (CoordinatesToWorldPoints(tileA, dimensions), 
			// 	CoordinatesToWorldPoints(tileB, dimensions), Color.green, 100);
			var line = BresenhamDrawLine(tileA, tileB);
			...
			foreach (var t in line)
				Draw(map, t, _random.Next(CorridorWidth.x, CorridorWidth.y));
		}

		void Draw(Span2D<int> map, Vector2Int linePoint, int radius) {
			for (var x = -radius; x <= radius; x++) {
				for (var y = -radius; y <= radius; y++)
					if (x * x + y * y <= radius * radius) { // inside of the circle
						var drawX = linePoint.x + x;
